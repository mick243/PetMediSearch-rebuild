...
- **가장 중요한 현재 사실 3가지**
  1. **로컬 `main` 이 원격과 갈라져 있습니다.** 측정 시점의 로컬 `main` = `4d2a7ef`(09-17)로 `origin/main`(`8c2de57`) 보다 **0 앞 · 2 뒤**였고, 그 위에 이 문서 커밋 둘(`284b903` · `a0b1456`)이 얹혀 **2 앞 · 2 뒤**가 됐습니다. 코드 커밋은 09-18 이후 없습니다. **`git pull --rebase` 로 풀어야 합니다.** → §2
...
|---|---|---|---|---|
| **`main`** | `a0b1456` (09-22) | **주 체크아웃** (미커밋 0) | — | **정본.** 마지막 커밋 둘은 이 문서 자신이고, 그 아래 코드 HEAD 는 `4d2a7ef`(09-17). `origin/main`(`8c2de57`) 대비 **2 앞 · 2 뒤** — `git pull --rebase` 필요 |
