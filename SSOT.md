...
- **가장 중요한 현재 사실 3가지**
  1. **로컬 `main` 이 원격과 갈라져 있습니다.** 측정 시점의 로컬 `main` = `4d2a7ef`(09-17)로 `origin/main`(`8c2de57`) 보다 **0 앞 · 2 뒤**였고, 그 위에 이 문서를 들인 커밋(`284b903`)이 하나 얹혀 **1 앞 · 2 뒤**가 됐습니다. 코드 커밋은 09-18 이후 없습니다. **`git pull --rebase` 로 풀어야 합니다.** → §2
...
|---|---|---|---|---|
| **`main`** | `284b903` (09-22) | **주 체크아웃** (미커밋 0) | — | **정본.** 마지막 커밋은 이 문서를 들인 것 자신이고, 그 아래 코드 HEAD 는 `4d2a7ef`(09-17). `origin/main`(`8c2de57`) 대비 **1 앞 · 2 뒤** — `git pull --rebase` 필요 |
...
- **원격이 앞서 있습니다.** 측정 시점 `git rev-list --left-right --count main...origin/main` = `0 2` — 09-18 의 `8c2de57`(PR #7 병합)이 로컬 main 에 안 들어와 있습니다. 이 문서를 들인 커밋 뒤로는 `1 2` 입니다.
...
| **R2** | 로컬 `main` 이 `origin/main` 과 갈라졌습니다 — 원격의 2커밋이 안 들어와 있고 그 위에 SSOT 신설 커밋이 얹혔습니다 | `rev-list --count main...origin/main` = 1/2 | 미해결 — `git pull --rebase` |
